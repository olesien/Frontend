...
const imgEl = game.querySelector("img");
...
const results = [
	{ name: "hey", url: "someURL" },
	{ name: "hey2", url: "someURL2" },
];
...
const toggleGameScreen = () => {
	...
	const choiceElementMap = choiceArray.map((choiceEl, index) => {
		...
		if (placementIndex === index) {
			imgEl.src = choicesEditable[randomizedChoice].url;
			console.log(choicesEditable[randomizedChoice].url);
...
const checkGuess = (clickedEl, id) => {
	...
	if (clickedEl.value === choicesEditable[randomizedChoice].name) {
		...
		choicesEditable = setChoicesEditable().filter(
...
gameContainer.addEventListener("click", (e) => {
	...
	if (clickedEl.classList.contains("restartBtn")) {
		choicesEditable = setChoicesEditable();

		toggleGameScreen();
		progressEl.innerText = `0% complete`;
